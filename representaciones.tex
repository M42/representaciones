...
hacer uso de el ratón como dispositivo de entrada para que el usuario interactúe
con la escena planteada, con el objetivo de llegar a una situación en la que se
da el bloqueo de Cardán, una vez llegados a este punto el usuario puede
interactuar con la esfera que aparece en la escena para realizar una
interpolación esférica (SLERP).

\input{euler.tex}

El script que permite el uso del ratón en el sistema es capaz de diferenciar que
rotor se ha pulsado, a partir de aquí es necesario rotarlos y mantener su
posición una vez se vuelva a pulsar en ellos, para ello se hace la composición
de las rotaciones generadas por el cuaternión actual del eje pulsado con el
cuaternión generado por la rotación del ángulo que viene determinado por el
tiempo en el que se pulse alguno de los botones del ratón.

\subsection{Uso de cuaternios}

En el programa se hace uso de los cuaternios para representar la rotación de una
determinada figura en el espacio, además están definidos los operadores que nos
permiten compararlos y componer rotaciones. Para resolver la situación de
bloqueo de Cardán se ha desarrollado un script que dados dos cuaterniones
realiza la interpolación esférica de uno a otro, esto afectará a la escena
haciendo que la figura salga del control de los ejes que bloquean el giro y
permitiendo a esta llegar a la situación deseada a través del camino más corto.
